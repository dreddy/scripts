...
parted ${DEVICE} --script mkpart EFI fat16 1MiB 512MiB
parted ${DEVICE} --script set 1 msftdata on

echo "Create OS partition"
parted ${DEVICE} --script mkpart LINUX btrfs 512MiB 100%
...
debootstrap --variant=minbase --arch amd64 --components=main,contrib,non-free buster ${ROOTFS} http://deb.debian.org/debian/
